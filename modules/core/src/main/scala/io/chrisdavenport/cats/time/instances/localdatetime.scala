...
trait localdatetime {
  ...
  implicit final val localdatetimeInstances =
    new Show[LocalDateTime] with Order[LocalDateTime] with Hash[LocalDateTime] with Eq[LocalDateTime]{
      ...
      override def eqv(x: LocalDateTime, y: LocalDateTime): Boolean = x.equals(y)
    }
}
